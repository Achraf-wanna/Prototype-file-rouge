			...
			if($guest != "" && $email && $phone != "" && $date_res != "" && $time != "" && $suggestions != "") {
				...
				if(count($table_array) < $table_count) {
					...
					if($check->num_rows) {
						...
						$insert = $db->query("INSERT INTO reservation(no_of_guest, email, phone, date_res, time, suggestions, fullname) VALUES('".$guest."', '".$email."', '".$phone."', '".$date_res."', '".$time."', '".$suggestions."', '".$fullname."')");
						...
						if($insert) {
							...
							$msg = "<p style='padding: 15px; color: green; background: #eeffee; font-weight: bold; font-size: 13px; border-radius: 4px; text-align: center;'>Reservation placed successfully. Your reservation code is $reserve_code. Please make sure to present your reservation code when arriving.</p>";
	...
	<div class="inner_content">
		...
		<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="hr_book_form">
			...
			<div class="form_group">
					...
					<input type="text" placeholder="Please Enter Your Full Name" name="fullname" required>
					
				</div>

				<div class="form_group">
					 
					 <label>No of Guest ( 6 Pers. Max )</label>
